...
async def run_child_bot(token: str, bot_id: str):
    ...
    async def handle_token(message: Message):
        # Проверяем, не находится ли пользователь в процессе ожидания токена
    ...
    async def stop_bot_callback(callback: CallbackQuery):
        # Для дочерних ботов просто показываем информацию
        await callback.message.edit_text(
            "⚠️ Эта функция доступна только в главном боте.\n"
            "Для остановки дочерних ботов используйте главный бот.",
            reply_markup=get_main_keyboard()
        )
        await callback.answer()
...
async def main():
    ...
    if not MAIN_BOT_TOKEN:
        logger.error("MAIN_BOT_TOKEN environment variable not set!")
        sys.exit(1)
    ...
    async def stats_callback(callback: CallbackQuery):
        ...
        if bot_manager.processes:
            stats_text += "\nЗапущенные боты:\n"
            for bot_id in bot_manager.processes.keys():
                process = bot_manager.processes[bot_id]
                status = "🟢 Работает" if process.is_alive() else "🔴 Остановлен"
                stats_text += f"• {bot_id}: {status}\n"
    ...
    for sig in [signal.SIGINT, signal.SIGTERM]:
        asyncio.get_event_loop().add_signal_handler(
            sig, lambda: asyncio.create_task(shutdown(bot, dp))
        )
